...
export async function getAirportStatus(icao, deps = {}) {
  ...
  const fetched = await fetchAircraftNear(airport, fetchImpl);
  if (fetched.error) return { error: fetched.error, updatedAt: Date.now() };

  const classified = classifyAircraft(airport, fetched.ac);
...
async function attachRoutes(items, fetchImpl) {
  ...
  await Promise.all(batch.map(async (it) => {
    ...
    try {
      const res = await fetchJson(`https://api.adsbdb.com/v0/callsign/${encodeURIComponent(it.callsign)}`, fetchImpl);
      status = res.status;
      if (status === 200) {
    ...
    it.route = route;
    // 404（未登録便）はしばらく再照会しない。5xx やタイムアウトは次回再試行。
    if (status === 200 || status === 404) routeCache.set(it.callsign, { at: Date.now(), route });
  }));
}
...
function normalizeAircraft(raw, airport) {
  ...
  return {
    ...
    lon: raw.lon,
    seen: raw.seen,
    special: SPECIAL_LIVERIES[reg] || null,
    phase: null,
    route: undefined,
  };
}
