/**
 * スペマウォッチ — サーバー側ロジック（Vercel Serverless Function から呼ばれる）
 *
 * データソース（無料・キー不要）
...
/**
 * 特別塗装機の辞書を取得する。DB（Supabase またはモック）が使えないときは
 * lib/liveries.js の静的な辞書にフォールバックするので、スペマウォッチは壊れない。
 * @returns {Promise<Object<string, Object>>} 登録記号（大文字）→ 塗装機
 */
export async function loadLiveryMap(deps = {}) {
  ...
  try {
    ...
    console.warn('[status] 塗装機DBの取得に失敗したため静的辞書を使います:', e && e.message ? e.message : e);
  }
