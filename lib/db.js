 ...
 *           getProfileById, getLiveryPageData, getSetting, setSetting, deleteOwnPhoto,
 *           setMockDisplayName,
 *           searchProfiles, listAdmins, setProfileRole, getPhotoForFinalize,
 *           assertPhotoPaths, DAILY_LIMITS
 *
...
import { isValidStoragePath, thumbPathFor } from '../public/js/validate.js';

export const LIVERY_CACHE_MS = 60 * 1000;
export const SETTINGS_CACHE_MS = 30 * 1000;
...
export function publicConfig() {
  ...
  return {
    ...
    // Google Identity Services の Web クライアント ID。公開してよい値（ブラウザの
    // <script> に埋め込むために作られている）。未設定なら null で、login.html が
    // 「Google ログインが未設定です」と出す。
 ...
 *
 * 本番では `http(s)://` と `data:` を **null** にする（DB に外部 URL が入り込んでも
 * 画面・OG 画像がそれを読みに行かないようにする多層防御。設計 §1）。
 * モックでは種データが `data:` の SVG なのでそのまま通す。
 */
export function publicPhotoUrl(path) {
  if (!path) return null;
  if (/^(https?:\/\/|data:)/.test(path)) return isMock() ? path : null;
  const base = supabaseUrl();
  if (!base) return null;
  return `${base}/storage/v1/object/public/${BUCKET}/${String(path).replace(/^\/+/, '')}`;
}

/** 1 日あたりの投稿上限（0004_hardening.sql の under_daily_limit と同じ値） */
export const DAILY_LIMITS = Object.freeze({ photos: 30, liveries: 20, reports: 20 });

/**
 * 写真行の保存先が規約どおりか確かめる（設計 §1）。
 * `{user_id}/{uuid}.jpg` と、その `.jpg` を `_thumb.jpg` に置き換えたサムネイルだけを許す。
 * 違反は dbError(400) を投げる（= 承認しない）。
 *
 * モックの種データ（`data:` の SVG）だけは例外として通す。
 * @param {{user_id?:string, userId?:string, storage_path?:string, storagePath?:string, thumb_path?:string, thumbPath?:string}} row
 */
export function assertPhotoPaths(row) {
  const r = row || {};
  const uid = String(r.user_id != null ? r.user_id : (r.userId != null ? r.userId : ''));
  const path = String(r.storage_path != null ? r.storage_path : (r.storagePath != null ? r.storagePath : ''));
  const thumb = String(r.thumb_path != null ? r.thumb_path : (r.thumbPath != null ? r.thumbPath : ''));

  if (isMock() && /^data:/.test(path)) return true;   // モックの種データ
  if (!uid) throw dbError(400, '写真の投稿者が不明です');
  if (!isValidStoragePath(uid, path)) throw dbError(400, `写真の保存先が不正です: ${path}`);
  if (thumb !== thumbPathFor(path)) throw dbError(400, `サムネイルの保存先が不正です: ${thumb}`);
  return true;
}
...
const MOCK_PROFILES = {
  user1: { id: 'user1', display_name: 'モック一般ユーザー', sns_url: 'https://x.com/mock_user1', role: 'user', email: 'user1@example.com' },
  admin: { id: 'admin', display_name: 'モック管理者', sns_url: null, role: 'admin', email: 'admin@example.com' },
};
...
/** モックの写真に振る uuid 風の文字列（本物の uuid と同じ形にして検証を通す） */
function mockUuid(n) {
...
function seedPendingDemo() {
  const livery = mock.addLivery({
    reg: 'JA03RJ', name: '検証用 特別塗装機（承認待ち）', airline: 'ANA', aircraft_type: 'B737-800',
  ...
  mock.addLivery({
    reg: 'JA999Z', name: '検証用 却下テスト塗装機', airline: 'JAL', aircraft_type: 'B767-300',
    note: '出典が無い例。却下の動作確認に使います。',
    created_by: 'user1', status: 'pending',
  });
  // 承認待ちの写真: 既存の承認済み塗装機（JA819A）と上のダミー塗装機に 1 枚ずつ
  ...
  mock.addPhoto({ livery_id: livery.id, user_id: 'user1', credit_name: 'モック一般ユーザー', taken_on: '2026-09-01', airport_icao: 'RJOO', caption: 'ダミー塗装機の写真', status: 'pending' });
...
export const mock = {
  ...
  addPhoto(data = {}) {
    const s = store();
    s.nextPhoto += 1;
    const uid = data.user_id || 'mock-user';
    // 既定のパスも 0004_hardening.sql の規約（{user_id}/{uuid}.jpg）に沿わせる
    const defaultPath = `${uid}/${mockUuid(s.nextPhoto)}.jpg`;
    const row = {
      id: s.nextPhoto,
      livery_id: Number(data.livery_id),
      user_id: uid,
      storage_path: data.storage_path || defaultPath,
      thumb_path: data.thumb_path || thumbPathFor(defaultPath),
  ...
  setProfile(id, patch = {}) {
    ...
    s.profiles[id] = next;
    if (patch.display_name !== undefined && patch.display_name !== cur.display_name) {
      mockSyncCredit(String(id), next.display_name);
    }
    return next;
  },
  deletePhoto(id) {
...
/** SQL の profiles_sync_credit() と同じ規則をモックで再現する（表示名 → クレジット） */
function mockSyncCredit(userId, displayName) {
  const name = String(displayName == null ? '' : displayName);
  let updated = 0;
  for (const p of store().photos) {
    if (String(p.user_id) === String(userId) && p.credit_name !== name) { p.credit_name = name; updated += 1; }
  }
...
/**
 * PostgREST のエラー本文（{code, message, details, hint}）を HTTP の状態コードに直す。
 * 呼び出し側（addReport など）が 409 / 403 / 400 をそのまま画面に返せるようにする。
 */
function restError(method, path, status, text) {
  let code = '';
  let message = '';
  try {
  ...
  const detail = message || String(text || '').slice(0, 200);
  if (code === '23505') return dbError(409, `既に登録されています: ${detail}`);
  if (code === '42501') return dbError(403, `権限がありません: ${detail}`);
  if (code === '23514') return dbError(400, `入力が不正です: ${detail}`);
  return new Error(`Supabase ${method} ${path}: HTTP ${status} ${String(text || '').slice(0, 200)}`);
}
...
async function rest(path, { method = 'GET', body, prefer, admin = false } = {}) {
  ...
  try {
    ...
    if (!res.ok) throw restError(method, path, res.status, text);
...
/**
 * 承認済みの塗装機を登録記号（大文字）をキーにしたオブジェクトで返す。60 秒キャッシュ。
...
export async function getApprovedLiveries() {
  ...
  for (const { row, photos } of rows) {
    ...
    const key = view.reg;
    // 同じ登録記号に複数の塗装機があるときは運航中（until が無い）を優先する
    const prev = map[key];
...
/**
 * 登録記号で承認済みの塗装機を 1 件返す（無ければ null）。
 * @param {string} reg
 */
...
export async function adminUpdateStatus({ type, id, action, reason, adminId } = {}) {
  ...
  if (isMock()) {
    ...
    // 承認する写真だけは保存先を再検証する（却下は通す＝不正な行を片付けられる）
    if (type === 'photo' && action === 'approve') assertPhotoPaths(row);
    ...
    if (type === 'photo' && action === 'approve') {
      const { data: before } = await rest(`photos?id=eq.${enc(numId)}&select=id,user_id,storage_path,thumb_path&limit=1`, { admin: true });
      const target = (before || [])[0];
      if (!target) throw dbError(404, `${type} が見つかりません: ${numId}`);
      assertPhotoPaths(target);
    }
...
// ---------------------------------------------------------------------------
// アプリ設定（app_settings。supabase/migrations/0003_settings.sql）
// ---------------------------------------------------------------------------

/**
 * 使ってよい設定キーと既定値。ここに無いキーは書き込めない
 * （service role キーは RLS を素通りするので、キーの検証もこの層で行う）。
 */
export const DEFAULT_SETTINGS = Object.freeze({
  auto_approve_photos: true,   // 写真を投稿と同時に公開する（既定 ON）
});

let settingsCache = null;   // { at, mode, map }

/** 設定は秘密ではないので読み取りは anon キーでよい（RLS: select は誰でも） */
async function loadSettings() {
  const mode = isMock() ? 'mock' : 'supabase';
  if (settingsCache && settingsCache.mode === mode && Date.now() - settingsCache.at < SETTINGS_CACHE_MS) {
    return settingsCache.map;
  }
  const map = { ...DEFAULT_SETTINGS };
  if (isMock()) {
    Object.assign(map, store().settings);
  } else {
    const { data } = await rest('app_settings?select=key,value');
    for (const row of data || []) {
      if (Object.prototype.hasOwnProperty.call(DEFAULT_SETTINGS, row.key)) map[row.key] = row.value;
    }
  }
  settingsCache = { at: Date.now(), mode, map };
  return map;
}

/** 設定のキャッシュを捨てる（書き込み直後とテストで使う） */
...
/**
 * 設定を 1 つ読む。30 秒キャッシュ。
 * 行が無ければ DEFAULT_SETTINGS（または引数の fallback）を返す。
 * バックエンドに届かなかった場合は例外を投げる（呼び出し側が安全側に倒せるように）。
 * @param {string} key
 * @param {*} [fallback]
 */
...
/**
 * 設定を 1 つ書く（管理者のみ。service role キー）。
 * @param {string} key DEFAULT_SETTINGS にあるキーだけ
 * @param {*} value 既定値と同じ型に揃える
 */
export async function setSetting(key, value) {
  const name = String(key == null ? '' : key).trim();
  if (!Object.prototype.hasOwnProperty.call(DEFAULT_SETTINGS, name)) {
    throw dbError(400, `設定キーが不正です: ${name || '(なし)'}`);
  }
  let v = value;
  if (typeof DEFAULT_SETTINGS[name] === 'boolean') {
    if (v === 'true') v = true;
    else if (v === 'false') v = false;
    if (typeof v !== 'boolean') throw dbError(400, `${name} は true / false で指定してください`);
  }

  if (isMock()) {
    store().settings[name] = v;
  } else {
    await rest('app_settings?on_conflict=key', {
      method: 'POST', admin: true, prefer: 'resolution=merge-duplicates,return=minimal',
      body: { key: name, value: v, updated_at: new Date().toISOString() },
    });
  }
  invalidateSettingsCache();
  return { ok: true, key: name, value: v };
}

// ---------------------------------------------------------------------------
// 管理者の管理（profiles.role）
// ---------------------------------------------------------------------------

export const ROLES = ['user', 'admin'];

function profileView(row, email) {
  return {
    id: row.id,
...
export async function listAdmins() {
  ...
  const { data } = await rest('profiles?role=eq.admin&select=id,display_name,role&order=display_name.asc&limit=100', { admin: true });
  return (data || []).map((row) => profileView(row));
}

/** 管理者の人数（最後の 1 人を外させないため） */
async function countAdmins() {
  if (isMock()) return Object.values(store().profiles).filter((p) => p.role === 'admin').length;
  return restCount('profiles', 'role=eq.admin');
}

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

/**
 * メールアドレスから auth.users を引いて profiles を返す（service role キーが必要）。
 * profiles はメールを持っていないので、Auth の管理 API を使う。見つからなければ null。
 */
async function findProfileByEmail(email) {
  ...
  let body = null;
  try {
    const res = await fetch(`${base}/auth/v1/admin/users?email=${enc(email)}`, {
  ...
  const users = Array.isArray(body) ? body : (body && Array.isArray(body.users) ? body.users : []);
  // email= の絞り込みを無視する版があるので、こちらでも突き合わせる
  const hit = users.find((u) => String((u && u.email) || '').toLowerCase() === email);
  if (!hit || !hit.id) return null;
  const profile = await getProfileById(hit.id);
  return profile
    ? { id: profile.id, display_name: profile.displayName, role: profile.role, email: hit.email }
    : { id: String(hit.id), display_name: '', role: 'user', email: hit.email };
}

/**
 * 表示名（部分一致）またはメールアドレスでユーザーを探す（管理者だけが呼ぶ）。
 * メールは Auth の管理 API から引いたときだけ返す（画面では管理者にしか見せない）。
 * @param {string} q
 * @returns {Promise<Array<{id:string, displayName:string, role:string, email?:string}>>}
 */
export async function searchProfiles(q) {
  const needle = String(q == null ? '' : q).trim();
  if (!needle) throw dbError(400, '検索語が必要です');
  if (needle.length > 100) throw dbError(400, '検索語が長すぎます');

  const email = EMAIL_RE.test(needle) ? needle.toLowerCase() : null;
  const out = [];
  const seen = new Set();
  const push = (row, mail) => {
    ...
    seen.add(id);
    out.push(profileView(row, mail));
  };

  if (isMock()) {
    const lower = needle.toLowerCase();
    for (const p of Object.values(store().profiles)) {
      if (String(p.display_name || '').toLowerCase().includes(lower)) push(p);
      else if (email && String(p.email || '').toLowerCase() === email) push(p, p.email);
    }
    return out.slice(0, 20);
  }

  // 表示名の部分一致（PostgREST の ilike。* がワイルドカード）
  const pattern = enc(`*${needle.replace(/[*,()]/g, ' ')}*`);
  const { data } = await rest(`profiles?display_name=ilike.${pattern}&select=id,display_name,role&order=display_name.asc&limit=20`, { admin: true });
  for (const row of data || []) push(row);

  if (email) {
    const byEmail = await findProfileByEmail(email);
    // メール一致は確実なので先頭に出す
    if (byEmail && !seen.has(String(byEmail.id))) {
      seen.add(String(byEmail.id));
      out.unshift(profileView(byEmail, byEmail.email));
    }
  }
  return out.slice(0, 20);
}
...
/**
 * profiles.role を変える。最後の管理者は外せない。
 * 本物では service role キーで PATCH する（profiles_guard_role は auth.uid() が NULL のとき
 * 素通しするので、service role からの更新は通る。0001_init.sql 参照）。
 * @param {string} userId
 * @param {'user'|'admin'} role
 */
export async function setProfileRole(userId, role) {
  const uid = parseUserId(userId);
  const next = String(role == null ? '' : role).trim();
  if (!ROLES.includes(next)) throw dbError(400, `role が不正です: ${role}`);

  const current = await getProfileById(uid);
  if (!current) throw dbError(404, `ユーザーが見つかりません: ${uid}`);
  if (current.role === next) return { ok: true, userId: uid, role: next, changed: false };

  if (current.role === 'admin' && next !== 'admin' && (await countAdmins()) <= 1) {
    throw dbError(400, '管理者が 0 人になるため外せません（先に別の管理者を追加してください）');
  }

  if (isMock()) {
    mockProfile(uid).role = next;
  } else {
    const { data } = await rest(`profiles?id=eq.${enc(uid)}&select=id,role`, {
      method: 'PATCH', body: { role: next }, prefer: 'return=representation', admin: true,
    });
    if (!(data || [])[0]) throw dbError(404, `ユーザーが見つかりません: ${uid}`);
  }
  return { ok: true, userId: uid, role: next, changed: true, displayName: current.displayName };
}

// ---------------------------------------------------------------------------
// 投稿直後の写真（api/photos.js の finalize）
// ---------------------------------------------------------------------------

/**
 * 写真 1 行の持ち主と状態だけを返す（無ければ null）。
 * 投稿者本人が finalize を呼べるかどうかの判定に使う。
 * @param {number|string} photoId
 */
export async function getPhotoForFinalize(photoId) {
  ...
  if (isMock()) {
    return view(store().photos.find((p) => p.id === id) || null);
  }
  const { data } = await rest(`photos?id=eq.${enc(id)}&select=id,user_id,status,livery_id,storage_path,thumb_path&limit=1`, { admin: true });
  return view((data || [])[0] || null);
}

/**
 * 投稿者本人が自分の写真を消す（**モック専用**）。
 *
 * 本物（Supabase）では、共有ページもマイページもブラウザから
 * Storage の remove → photos の delete を直接呼ぶ（RLS の photos_delete が本人だけを通す）。
 * サーバー経由で消すと Storage の実ファイルが残ってしまうので、この関数は使わない。
 * モックには Storage が無く、共有ページが見ている写真は lib/db.js のメモリ内ストアに
 * あるため（ブラウザ側の public/js/mockdb.js からは触れない）、この経路が要る。
 *
 * 代表写真だったときは 0005_credit_and_delete.sql の photos_after_delete と同じく
 * 次の承認済み写真を代表にする（mock.deletePhoto が mockDecidePrimary を呼ぶ）。
 *
 * @param {number|string} photoId
 * @param {string} userId 依頼者。他人の写真は 404（存在を教えない）
 */
export async function deleteOwnPhoto(photoId, userId) {
  const id = parsePositiveId(photoId, 'photoId');
  const uid = parseUserId(userId);
  if (!isMock()) throw dbError(400, 'この経路はモック専用です（本物はブラウザから直接削除します）');
  const row = store().photos.find((p) => p.id === id);
  if (!row || String(row.user_id) !== String(uid)) throw dbError(404, '写真が見つかりません');
  mock.deletePhoto(id);
  invalidateCache();
  return { ok: true, id, liveryId: row.livery_id };
}

/**
 * 表示名の変更をサーバー側のモックストアに反映する（**モック専用**）。
 *
 * 本物（Supabase）では profiles の update が 0005_credit_and_delete.sql の
 * profiles_sync_credit トリガーを動かすので、この関数は要らない。
 * モックはブラウザ側（public/js/mockdb.js）とサーバー側（このファイル）で
 * プロフィールのストアが別なので、共有ページのクレジットを追随させるのに使う。
 *
 * @param {string} userId
 * @param {string} displayName public/js/validate.js の validateDisplayName と同じ条件
 */
export async function setMockDisplayName(userId, displayName) {
  if (!isMock()) throw dbError(400, 'この経路はモック専用です');
  const uid = parseUserId(userId);
  const name = String(displayName == null ? '' : displayName).trim();
  if (!name) throw dbError(400, '表示名を入れてください');
  if (name.length > 40) throw dbError(400, '表示名は 40 字以内にしてください');
  if (/[<>]/.test(name)) throw dbError(400, '表示名に < > は使えません');
  const profile = mock.setProfile(uid, { display_name: name });
  invalidateCache();
  return { ok: true, userId: uid, displayName: profile.display_name };
}
...
function mockReportTarget(row) {
  ...
  return {
    label: `写真 #${p.id}（${l ? l.name : '塗装機 #' + p.livery_id}・撮影 ${p.credit_name}）`,
    status: p.status,
    thumbUrl: publicPhotoUrl(p.thumb_path),
  };
}

/** 通報の対象の見出し（本物）。塗装機・写真をまとめて 1 回ずつ引く */
async function reportTargets(rows) {
  ...
  if (photoIds.length) {
    ...
    for (const p of data || []) {
      out[`photo:${p.id}`] = {
        label: `写真 #${p.id}（${(p.liveries && p.liveries.name) || '塗装機 #' + p.livery_id}・撮影 ${p.credit_name}）`,
        status: p.status,
        thumbUrl: publicPhotoUrl(p.thumb_path),
      };
    }
...
/**
 * 登録記号で承認済みの塗装機とその承認済み写真を返す（代表写真の差し替え画面用）。
 * @param {string} reg
 */
...
/** 運航中（until が無い）を先に、次に新しい順。同じ登録記号に複数の塗装機があるときのタブ順 */
 ...
 * 塗装機は運航中を先頭にした承認済みの全件（タブ表示）、写真は承認済みを代表写真から順に並べる。
 *
...
export async function getLiveryPageData(reg) {
  ...
  return {
    ...
    photos: approved.map(({ p, liveryId }) => ({
      id: p.id,
      liveryId,
      // userId / storagePath / thumbPath は共有ページの「自分の写真を削除」に必要。
      // profiles は公開（RLS で誰でも読める）なので user_id は秘密ではない。
      // メールや role は決してここに入れない（test/security.test.js が固定している）。
...
/** 重複通報のエラー（画面はこの文言をそのまま出す） */
const DUPLICATE_REPORT = 'この対象は既に通報済みです';

/** 未解決の通報を出した「人数」を数える（同じ人が何回出しても 1 人。設計 §2） */
function distinctReporters(rows) {
  const set = new Set();
  for (const r of rows || []) {
    const id = r && r.reporter_id != null ? String(r.reporter_id) : '';
    if (id) set.add(id);
  }
  return set.size;
}
...
/**
 * 通報を 1 件入れる。写真に未解決の通報が 3 **人**ぶん以上ついたら
 * 写真を承認待ちに戻して非表示にする（設計書 §5-4 / セキュリティ強化 §2）。
 *
 *  - 同じ人が同じ対象に出せる未解決の通報は 1 件まで（409）
 *  - 1 人 1 日 20 件まで（429）
 *
...
export async function addReport({ targetType, targetId, reason, reporterId } = {}) {
  ...
  if (isMock()) {
    ...
    if (uid) {
      const today = s.reports.filter((r) => String(r.reporter_id) === uid && String(r.created_at) > since).length;
      if (today >= DAILY_LIMITS.reports) {
        throw dbError(429, `通報は 1 日 ${DAILY_LIMITS.reports} 件までです。時間をおいてお試しください`);
      }
      const dup = s.reports.some((r) => !r.resolved && r.target_type === targetType
        && r.target_id === id && String(r.reporter_id) === uid);
      if (dup) throw dbError(409, DUPLICATE_REPORT);
    }
    inserted = mock.addReport({ target_type: targetType, target_id: id, reason: text, reporter_id: uid });
    count = distinctReporters(s.reports.filter((r) => !r.resolved && r.target_type === targetType && r.target_id === id));
    ...
    if (uid) {
      // 1 日の上限（reports はサービスキー経由なので RLS ではなくここで数える）
      const today = await restCount('reports', `reporter_id=eq.${enc(uid)}&created_at=gt.${enc(since)}`);
      if (today >= DAILY_LIMITS.reports) {
        throw dbError(429, `通報は 1 日 ${DAILY_LIMITS.reports} 件までです。時間をおいてお試しください`);
      }
      const { data: dup } = await rest(`reports?select=id&resolved=is.false&target_type=eq.${enc(targetType)}&target_id=eq.${enc(id)}&reporter_id=eq.${enc(uid)}&limit=1`, { admin: true });
      if ((dup || []).length) throw dbError(409, DUPLICATE_REPORT);
    }
    ...
    try {
      ...
      // reports_one_open_per_reporter（部分ユニーク索引）に当たった＝重複通報
      if (e && e.status === 409) throw dbError(409, DUPLICATE_REPORT);
      throw e;
    }

    // PostgREST は count(distinct) を持たないので、reporter_id を取って JS で数える
    const { data: openRows } = await rest(`reports?select=reporter_id&resolved=is.false&target_type=eq.${enc(targetType)}&target_id=eq.${enc(id)}&limit=50`, { admin: true });
    count = distinctReporters(openRows);
    if (targetType === 'photo' && count >= REPORTS_TO_HIDE && target.status === 'approved') {
...
/** hex が空の塗装機（却下済みは除く）を古い順に limit 件 */
...
/**
 * そのユーザーの写真のクレジットを今の表示名に付け替える。
 *
 * 0005_credit_and_delete.sql の profiles_sync_credit トリガーが改名を自動で反映するので、
 * 通常はこれを呼ぶ必要は無い。**修復用**（トリガー導入前に改名された行や、
 * 何かの理由でずれた行を揃え直す）として残してある。
 * @param {string} userId
 */
