...
export async function getPhotoForFinalize(photoId) {
  ...
  if (isMock()) {
    ...
    return row ? { id: row.id, userId: row.user_id, status: row.status, liveryId: row.livery_id } : null;
  }
  const { data } = await rest(`photos?id=eq.${enc(id)}&select=id,user_id,status,livery_id&limit=1`, { admin: true });
  const row = (data || [])[0];
  return row ? { id: row.id, userId: row.user_id, status: row.status, liveryId: row.livery_id } : null;
}
