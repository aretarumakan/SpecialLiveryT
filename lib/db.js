...
const MOCK_PROFILES = {
  user1: { id: 'user1', display_name: 'モック一般ユーザー', sns_url: null, role: 'user' },
...
function store() {
  if (!mockStore) {
    ...
    // `npm run dev` は MOCK_SEED_PENDING=1 を付けるので /admin.html に触れる材料が入る
    if (env('MOCK_SEED_PENDING') === '1') seedPendingDemo();
  }
  return mockStore;
}
